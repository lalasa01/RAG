...
Ingestion:
 First we ingested a PDF file which has the information about Spotify and how it was using AI.  This comes under the documentation step where the file with information is given on which later querying is done.
...
Synthesis HF_TOKEN is used for authentication to use the services its providing through llama_index which helps in the process of embedding and connecting it with an LLM
...
 Next indexing was done where the query was indexed to solve it. Synthesis:
